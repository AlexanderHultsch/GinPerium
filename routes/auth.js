...
function requireString(value, label, { min = 1, max = 200 } = {}) {
  if (typeof value !== 'string' || value.trim().length < min || value.length > max) {
    throw apiError('VALIDATION', `${label} muss zwischen ${min} und ${max} Zeichen lang sein.`);
  }
  return value.trim();
}
...
export function createAuthRouter({ repo, auth, hashPassword, verifyPassword, newId, now, rateLimiter }) {
  ...
  router.post('/register', limit, (req, res) => {
    const username = requireString(req.body?.username, 'Der Benutzername', { min: 3, max: 32 });
  ...
  router.post('/login', limit, (req, res) => {
    const username = requireString(req.body?.username, 'Der Benutzername');
